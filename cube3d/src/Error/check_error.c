...
/*   Updated: 2024/09/05 15:22:37 by joncurci         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_map(char **temp, int start, int x, int y)
{
	...
	if (y >= ft_strlen(temp[x]) - 1)
		y = 0;
	if (x != start && temp[x - 1][y] != '1' && temp[x - 1][y] != '2' && temp[x - 1][y] != 'C' && temp[x - 1][y] != 'Z' && temp[x - 1][y] != 'D')
		if (check_map(temp, start, x - 1, y))
			return (1);
	if (y != 0 && temp[x][y - 1] != '1' && temp[x][y - 1] != '2' && temp[x][y - 1] != 'C' && temp[x][y - 1] != 'Z' && temp[x][y - 1] != 'D')
		if (check_map(temp, start, x, y - 1))
			return (1);
	if (temp[x][y + 1] && temp[x][y + 1] != '1' && temp[x][y + 1] != '2' && temp[x][y + 1] != 'C' && temp[x][y + 1] != 'Z' && temp[x][y + 1] != 'D')
		if (check_map(temp, start, x, y + 1))
			return (1);
	if (temp[x + 1][y] && temp[x + 1][y] != '1' && temp[x + 1][y] != '2' && temp[x + 1][y] != 'C' && temp[x + 1][y] != 'Z' && temp[x + 1][y] != 'D')
		if (check_map(temp, start, x + 1, y))
			return (1);
	return (0);
}
...
int	check_map(char **temp, int start, int x, int y)
{
	// Vérification des limites pour éviter un dépassement de tableau
	if (x < 0 || y < 0 || x >= ft_tablen(temp) || y >= ft_strlen(temp[x]))
		return (0);

	// Si la case est un '0' ou une position valide et que c'est un bord incorrect, renvoie une erreur
	if ((temp[x][y] == '0' || is_position(temp[x][y])) && check_boder(temp, x, y))
		return (1);

	// Si la case est déjà marquée, arrête la récursion
	if (temp[x][y] == '2')
		return (0);
	...
	// Vérification récursive des cases adjacentes
	...
	if (temp[x][y + 1] && temp[x][y + 1] != '1' && temp[x][y + 1] != '2')
		if (check_map(temp, start, x, y + 1))
			return (1);
	if (temp[x + 1] && temp[x + 1][y] != '1' && temp[x + 1][y] != '2')
		if (check_map(temp, start, x + 1, y))
			return (1);
